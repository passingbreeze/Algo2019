...
class womadCrwl(crwlframe):
    ...
    def _getContents(self, num):
        ...
        if pURL.status_code == 200:
            ...
            replNick = post.find_all("li", re.compile("comment"))
            print("check : ", replNick)
            ...
            # if len(content) > 0:
            #     with open("womad_Repl_0%d.txt" % self.textNum, "a", encoding='utf-8') as wf:
            #         wf.write("원글 작성자 : "+ nickName + "원글 url : " + self.pURL + " _# "+ content[0] + "$$\n")
    ...
    def getRepl(self):
        while len(self.womad_16_18) > 0:
            ...
            self._getContents(temp)
            print("womad_Repl_0%d.txt -> out!" % self.textNum)
            self.textNum+=1
        print("Complete!\n")
